"""
Created on Mon Nov 11 04:12:39 2024
...
class Linear_relu(nn.Module):
    ...
    #similiar to mamba block but without SSM & conv
    def forward(self, x):
        residual = x
        x = self.linear(x)
        #x = F.relu(x)  # 增加非线性激活，提高表示能力
        x = self.norm(x)
        #x = self.linear2(x)
        return x + residual  # 仍然保留残差连接

# ------------------------------------------------------------------------------
# 3) 主体网络：BrainMamba1_parallel，替换 input_embedding 为多分支结构
# ------------------------------------------------------------------------------
class BrainUnet_multibranch(nn.Module):
    """
    ...
    def __init__(self, in_channels, out_channels, d_state, d_conv, expand, scale_factor=1, logger=None):
        super(BrainUnet_multibranch, self).__init__()
        ...
        self.encoder1 = nn.Sequential(
            ...
            Linear_relu(d_model=base_channels, d_state=d_state, d_conv=d_conv, expand=expand),
        )
        ...
        self.bottleneck = nn.Sequential(
            ...
            Linear_relu(d_model=256 // self.scale_factor, d_state=d_state, d_conv=d_conv, expand=expand),
            nn.Linear(256 // self.scale_factor, 256 // self.scale_factor),
        )
